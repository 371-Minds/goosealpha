...
if [ "$ENABLE_API_MOUNT" = "true" ]; then
  docker run -d \
    ...
    -e MINDPORT_TLD="${MINDPORT_TLD:-localhost}" \
    -e MINDPORT_PROXY_PORT="${MINDPORT_PROXY_PORT:-1355}" \
    -e MINDPORT_DNS_PROVIDER="${MINDPORT_DNS_PROVIDER:-}" \
    -e OPENPROVIDER_USERNAME="${OPENPROVIDER_USERNAME:-}" \
    -e OPENPROVIDER_PASSWORD="${OPENPROVIDER_PASSWORD:-}" \
    -e PORKBUN_API_KEY="${PORKBUN_API_KEY:-}" \
    -e PORKBUN_SECRET_KEY="${PORKBUN_SECRET_KEY:-}" \
    -e NAMECHEAP_API_USER="${NAMECHEAP_API_USER:-}" \
    -e NAMECHEAP_API_KEY="${NAMECHEAP_API_KEY:-}" \
    -e NAMECHEAP_USERNAME="${NAMECHEAP_USERNAME:-}" \
    -e NAMECHEAP_CLIENT_IP="${NAMECHEAP_CLIENT_IP:-}" \
    -e FREENAME_API_KEY="${FREENAME_API_KEY:-}" \
